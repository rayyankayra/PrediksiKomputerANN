...
df = pd.read_csv(r"C:\Users\KAIRA\Downloads\LISTRIKLAB - Sheet1(5).csv")

# Menyusun data:
# X adalah fitur input (ampere, watt, volt)
X = np.column_stack((df["ampere"], df["watt"], df["volt"]))
# y adalah target output (jumlah komputer YouTube dan idle)
y = np.column_stack((df["youtube"], df["idle"]))
...
def train_model(model, train_loader, test_loader, epochs=50):
    ...
    for X_batch, y_batch in train_loader:
        # Cek NaN di input batch
        if torch.isnan(X_batch).any():
            print("NaN detected in input batch during training!")
            print(X_batch)
            return
        # Cek NaN di target batch
        if torch.isnan(y_batch).any():
            print("NaN detected in target batch during training!")
            print(y_batch)
            return
...
for name, model in models.items():
    ...
    with torch.no_grad():
        for X_batch, y_batch in test_loader:
            outputs = model(X_batch)
            if torch.isnan(outputs).any():
                print("NaN detected in model output during evaluation!")
                break
